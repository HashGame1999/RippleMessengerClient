...
import { PrivateChatAESString } from './MessengerConst'
...
function genAESKey(shared_key, address1, address2, sequence) {
  ...
  const salt = CryptoJS.SHA512(PrivateChatAESString + address12 + sequence)
  const derivedKeyLength = 32

  const aesKey = hkdf(shared_key, salt, derivedKeyLength)

  console.log('派生的AES-256密钥（十六进制）：', aesKey.toString());
